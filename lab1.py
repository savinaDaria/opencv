...
onlydirs = [f for f in listdir("alphabet") if isdir(join("alphabet", f))]

def avg_for_files_in_dir(hash1,dirname):
    imgfiles= [f for f in listdir("alphabet/"+dirname) if isfile(join("alphabet/"+dirname, f))]
    allcomparesfordir = []
    for ex in imgfiles:
        img = cv2.imread('alphabet/'+dirname+'/'+ ex)
...
letters=letters_extract("12.png")
